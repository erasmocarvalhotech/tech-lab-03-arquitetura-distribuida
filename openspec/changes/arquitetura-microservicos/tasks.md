...
- [ ] 3.3 Listener consumindo `mb-techlab-stock-queue-product-changed` — cria `SaldoEstoque` zerado quando header `event-type=product-created`; em falha, aplica a regra de retry via `x-death` (Decisão 7: até 3 tentativas — 1 inicial + 2 via `-delayed` — depois publica em `-failed`)
...
- [ ] 3.8 Testes: reserva bem-sucedida decrementa saldo; reserva com saldo insuficiente (sem conflito) publica `rejected` sem retry; reserva com conflito de concorrência (`OptimisticLockException`) não publica nada e a mensagem é reprocessada via `-delayed`; falha persistente é reprocessada exatamente 3 vezes (1 inicial + 2 retries) e então cai em `-failed`
...
- [ ] 4.6 Listener consumindo `mb-techlab-order-queue-reservation-processed` — atualiza status para `CONFIRMADO` (confirmed) ou `REJEITADO_SEM_ESTOQUE` (rejected); em falha, aplica a regra de retry via `x-death` (Decisão 7: 3 tentativas no total)
...
- [ ] 5.5 Forçar falha persistente de um consumidor e verificar que a mensagem circula pela fila `-delayed` por 2 ciclos (3 tentativas de processamento no total) e, na 3ª falha, cai na fila `-failed`
